# Firebase 설정 가이드 — 방명록 · 사진 좋아요

방명록과 사진 좋아요를 **모든 관람객이 공유**하려면 Firebase(Firestore)를 연결해야 합니다.
...
- 로그인이 없어 완벽한 도배 차단은 어렵습니다. 현재 클라이언트에서 20초 작성 간격 제한과 좋아요 1인 1회(브라우저 기준) 제한을 두었습니다.

## 비용

무료(Spark) 요금제로 충분합니다 — 하루 읽기 5만/쓰기 2만 회, 저장 1GB. 학교 전시 규모를 크게 웃돕니다.
