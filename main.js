...
function buildCinema(def, roomGroup) {
  ...
  // "요나고역 9와 3/4승강장" 표시 — 스크린 맨 아래에 걸쳐 입구 개구부(빈틈)를 덮는다. 비밀의 방 단서.
  // 영상과는 아주 살짝(≈0.03m)만 겹치고, 나머지는 스크린 하단~바닥의 빈틈을 가린다.
  const platLabel = textPlane([
    { text: 'YONAGO STATION · PLATFORM 9¾', size: 0.2, weight: 700, spacing: 0.08, color: '#f2d675' },
    { text: '요나고역 9와 3/4승강장 · 米子駅9と3/4番線', size: 0.1, color: '#e8e6e1' },
...
// 시네마 동쪽 벽 포털(표지) → 어두운 통로 안의 점프맵 → 비밀의 방.
...
function buildMuseum(manifest) {
  ...
  // 비밀의 방 챌린지(요나고역 9와 3/4승강장) — 시네마 동쪽 벽 뒤에 짓는다.
...
function solveChallenge() {
  ...
  showHint('스크린이 열렸어요! 화면 속으로 들어가 발판을 건너세요<br>スクリーンが開いた！画面の中へ入り、足場を渡ろう');
  startHofWatch();
}
